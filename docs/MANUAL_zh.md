...
版本 **5.8.2** · `linux/amd64` · 離線檔 36.2 MB · 弱點掃描 CRITICAL 0 / HIGH 0 / MEDIUM 0
...
**5.6 —— 「斷點如何影響」在真實部署也算得出來**
- 那一頁以前靠示範模式抽好的走法來判斷「這一刀解釋得了哪幾條斷線」；到了
  公司沒有那份資料，每一條都會變成「解釋不了」，跟結論帶自相矛盾。
- 現在改成用**候選路徑**：兩端 DC 之間的實體路徑有幾條、幾條經過這一刀。
  那正是定位公式在用的比例（f(x,e)），卡片上會標「3／5 條路徑經過斷點」。
  我們確實不知道每條邏輯連線實際走哪一段，畫面的措辭也改成「可能路徑經過它」。
- 「解釋不了」現在的意思很明確：兩端之間**沒有任何一條**路徑經過這一刀。

**5.6.1 / 5.6.2**
- 現場圖點一台設備後的詳情卡，改成**一台對端一組**：對端名稱當標題出現一次，
  底下列它的每個 port，斷的組排最前面。以前一列一 port，同一台會出現好幾列。
- 「斷點如何影響」的措辭改成**候選路徑**，並說明上限：兩端 DC 之間
  **4 跳以內**的實體路徑（最多列 20 條）有幾條經過這一刀。更長的繞路確實
  存在但不列入 —— 流量不會在有直達光纖時繞五跳，而定位公式用的就是這一組。
...
**5.8.2**
- 修一個 bug：從現場圖「看替代路徑 →」跳到「斷點如何影響」後按「顯示全部」，
  切去別的頁籤再回來，又只剩那台設備的幾對。現在「顯示全部」會一併清掉
  跳轉時記下的設備，回來就是全部。
...
```bash
docker pull coolguazi/fiber-cut-localizer:5.8.2
```
...
>
> ```bash
> docker pull --platform linux/amd64 coolguazi/fiber-cut-localizer:5.8.2
...
```bash
docker pull --platform linux/amd64 coolguazi/fiber-cut-localizer:5.8.2
docker save coolguazi/fiber-cut-localizer:5.8.2 | gzip > fcl-5.8.2.tar.gz
```

把 `fcl-5.8.2.tar.gz` 拷到公司主機（約 36 MB），然後：

```bash
gunzip -c fcl-5.8.2.tar.gz | docker load
```
...
services:
  app:
    image: coolguazi/fiber-cut-localizer:5.8.2
