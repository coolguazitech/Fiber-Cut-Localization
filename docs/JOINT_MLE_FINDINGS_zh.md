...
## 9. 同一對 DC 的多條 peer 要不要各算一票

### 問題

`f(x, e)`（一條邏輯連線的候選物理路徑有多少比例經過光纖 e）**只跟兩端的 DC
有關**。所以同一對 DC 之間的每一條 peer 貢獻**完全相同**的項。實際資料裡這件事
很不平均：
...
也就是說，一對佈了 11 條平行線路的 DC，它的證據在概似值裡被算了 11 次 ——
而平行線路的數量是機房佈線的結果，跟斷點在哪無關。乍看之下排名會偏向「服務
高 peer 數 DC 對」的那幾條光纖。

### 這是不是錯的，取決於那幾條線怎麼走

- **各走各的實體路徑** → 它們是 m 次**獨立**觀測，算 m 次是正確的 Bayesian
  更新，壓掉就是丟證據。
- **共用同一條管道** → 它們是同一件事的 m 份副本，算 m 次是把一次觀測數了 m 遍。

所以不能用直覺決定。引入一個指數 λ，每條 peer 的權重 = 1/m^λ：λ=0 是各算一票
（原版），λ=1 是同一對 DC 合計一票，中間是統計上處理叢集觀測的「有效樣本數」
修正。

### 量測

`experiment_peer_weighting.py`，用 ground truth 掃過四種情境，每格 400 次試驗，
拓樸 9 個 DC / 12 段光纖，N 用真實系統匯出的 82 條（多重性 1～11）：

| 平行線路佈線 | 物理路徑分佈 | λ=0（原版） | λ=0.5 | λ=1 |
|---|---|---|---|---|
| 各走各的 | 均勻抽 | 82.2% | **85.5%** | 60.0% |
| 共用管道 | 均勻抽 | 37.9% | 41.5% | **42.8%** |
| **各走各的** | **偏好短路徑 α=2** | **97.5%** | 86.0% | 58.2% |
| 共用管道 | 偏好短路徑 α=2 | 59.9% | 61.1% | **61.9%** |

（top-1 命中率。第三列是最接近現場的假設：平行線路通常刻意分開佈放，而物理
路徑分佈偏好短 hops —— 那正是本文件第 1 節的主題。）

### 結論

**維持 λ=0。** 在最接近現場的那一格，原版以 97.5% 大勝；兩種設定給出不同答案的
161 次裡，λ=0 對了 158 次。

原因不難理解：短路徑偏好讓斷線集中在真正經過那一刀的 peer 上，於是多重性帶來的
是**真實的重複取樣**。把它壓掉等於丟掉證據 —— 這跟第 2 節「均勻假設讓 identifiability
翻盤」是同一類錯誤的反面：那裡是假設太樂觀，這裡是修正過了頭。

λ 調大只在「平行線路共用管道」時划算，而且只贏約 2 個百分點，卻要在各走各的
情況下賠掉 11～39 個百分點。

### 但這件事看得出來，不必用猜的

共用管道的話，一刀下去那一對 DC 的 peer 會**全斷或全不斷**；各走各的話會出現
「斷 3 條、活 5 條」。`peer_routing_diversity()` 就是數這個，回一句建議。
部署時可以用 `BAYES_PEER_WEIGHT` 覆寫（預設 0）。

順帶修掉一個相關的量尺問題：信心分級用的 `gap_per_observation`，分母原本是
「斷線條數」，現在改成**有效觀測數**（也就是概似值裡真正計入的那些）。兩者在
λ=0 時幾乎相同，但分子一旦加權而分母沒有，同一份證據會因為換了加權而看起來
變弱 —— 那不是證據變少，是不再被重複計算。

實驗腳本：`experiment_peer_weighting.py`（local-only，不在 public repo）。

---

## 10. 觀測失準時排名撐到哪裡 —— 兩條韌性曲線

論文要主張的是：**即使觀測跟真相有落差，排名還是可用的**。落差有兩種方向，
韌性要分開量。

| | 落差的方向 | 對概似值的影響 |
|---|---|---|
| (a) log 不完整 | 斷了但沒收到 log | 那條 peer 被當成 **silent** —— 不只是少一票，是投給了反方 |
| (b) 平行線路相關 | 同一對 DC 的 peer 共用實體路徑，一起斷、一起沉默 | 票數看起來多，**獨立資訊**沒有那麼多 |

設定：偏好短路徑 α=2、真實的 82 條 N、每點 200 次試驗。
...
**(c) 兩者同時（漏 30% ＋ 共用 50%）**：top-1 70.7%、top-3 92.9%。

### 讀法

1. **可以主張的是 top-3，不是 top-1。** 兩種落差各自到一半的程度，真正的斷點
   仍然以 ≥93% 留在前三名；兩者疊加也還有 92.9%。這正是「排名可能性」而不是
   「指出那一條」的意思 —— 論文的措辭應該是前者。
2. **兩種落差的壞法不同。** 漏 log 在 30% 以內幾乎無感（silent 項有 γ=0.8 的折扣，
   正是在對沖這件事），過了一半才崩。平行線路相關則是一開始就掉（25% 就少 16 個
   百分點），因為它削的是獨立資訊的**總量**，任何加權都補不回來 —— 見第 9 節。
3. **「切下去只斷部分」是好消息，不是失準。** 同一對 DC 的 8 條 peer 斷了 3 條、
   活了 5 條，代表它們各走各的路徑，是 8 次獨立觀測 —— 模型就是為這種情況寫的。
   真正削弱模型的是 8 條**一起**斷：那時 8 票只值 1 票。

實驗腳本在 `experiment_peer_weighting.py` 同一支（local-only）。
